 ...
 * (b) the AS JWK set is served (the RS256 signing key); (c) the local user store (V32) is seeded
 * and the {@link UserDetailsService} resolves the seed user with its {@code ROLE_*} authorities and
 * a BCrypt-verifiable password. This runs in the {@code dev,e2e} profiles (NOT {@code test}), so
 * the AS chains and the {@code DevUserSeeder} are active — exactly the production path minus the
 * external origin.
 *
 * <p>Its own Postgres container ({@code @Testcontainers}/{@code @Container}) is used rather than
 * the shared {@code test}-profile singleton, because this test intentionally does not run under
 * {@code test} (it needs the real security chains, not {@code TestSecurityConfig}).
 */
...
class AuthorizationServerIntegrationTest {
  ...
  void seedsTheLocalUserStoreAndResolvesRolesAndPassword() {
    ...
    assertThat(director.getAuthorities()).anyMatch(a -> a.getAuthority().equals("ROLE_DIRECTOR"));
    // The stored credential is a BCrypt hash of the dev password (never plaintext — BR4).
    assertThat(passwordEncoder.matches(DevUserSeeder.DEV_PASSWORD, director.getPassword()))
        .isTrue();
  }

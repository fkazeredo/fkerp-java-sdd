 ...
 *
 * <p><strong>Consent</strong> ({@link com.fksoft.domain.marketing.Consent}) is an append-only log
 * (DL-0056): every decision of a subject is an immutable row; the current state is the most recent
 * row per {@code (subjectType, subjectId, purpose)}; revoking or re-consenting is a new row, never
 * an update (BR1). No campaign send may reach a subject without a {@code GRANTED} consent for that
 * purpose (BR2) — the filter runs before enqueuing and the suppressed recipients are counted, never
 * a global error.
 *
 * <p><strong>Segment</strong> ({@link com.fksoft.domain.marketing.Segment}) is defined by criteria
 * over already-existing data (minimization, BR3): the criteria are a validated jsonb against a
 * closed catalog of allowed fields (DL-0059). <strong>Campaign</strong> ({@link
 * com.fksoft.domain.marketing.Campaign}) sends through the {@link
 ...
 * <strong>Attribution</strong> ({@link com.fksoft.domain.marketing.Attribution}) links a campaign
 * code to a booking (BR5/DL-0057): a code→booking intake plus a confirmation when {@link
 ...
 * so a subject is never silently re-included in a future send. The implementation types (the
 * aggregates and their repositories) live in this same package marked {@link

 ...
 *
 * <p><strong>RepresentedBrand</strong> ({@link com.fksoft.domain.portfolio.RepresentedBrand}) is
 * the reference "which brand", with a unique {@code brandRef} (value) and an ACTIVE/INACTIVE status
 * (BR1). <strong>RepresentationContract </strong> ({@link
 * com.fksoft.domain.portfolio.RepresentationContract}) holds the validity window, the Compliance
 * {@code documentId} (value, never an FK) and the reference terms (jsonb, not prices — BR6);
 * selling a brand without an in-force contract only <strong>alerts</strong>, it never blocks
 ...
 *
 * <p><strong>BrandGoal</strong> ({@link com.fksoft.domain.portfolio.BrandGoal}) defines a VOLUME or
 * REVENUE target per (brand, period); the <strong>realized</strong> is a read-model projection
 * (BR4/DL-0062) built from sales events — {@code BookingConfirmed} (VOLUME) and {@code
 * SpreadRealized} (REVENUE, BRL) — matched to a brand by a Portfolio-owned sale-attribution intake
 * ({@link com.fksoft.domain.portfolio.BrandSaleAttribution}), <strong>without changing the sale
 * event</strong>. The projection ({@link com.fksoft.domain.portfolio.BrandRealized}) is idempotent
 * per {@code (metric, sourceRef)}.
 *
 ...
 * module graph stays <strong>acyclic</strong> (Spring Modulith verify). The implementation types
 * (the aggregates, the projection and their repositories) live in this same package marked {@link

 ...
 *
 * <p><strong>BrandGoal</strong> (slice 8g-2) defines a VOLUME or REVENUE target per (brand,
 * period); the <strong>realized</strong> is a read-model projection (BR4/DL-0062) built from sales
 * events — {@code BookingConfirmed} (VOLUME) and {@code SpreadRealized} (REVENUE, BRL) — matched to
 * a brand by a Portfolio-owned sale-attribution intake, <strong>without changing the sale
 * event</strong>. The projection is idempotent per {@code (metric, sourceRef)}.
 *
 * <p>Spring Modulith application module (the 17th). Dependencies: it consumes the {@code booking}
 * {@code BookingConfirmed} and the {@code reconciliation} {@code SpreadRealized} events only (never
 * those modules' facades nor internals — DL-0062), plus the {@code money}/{@code error} kernels; no
 * other business module depends back on Portfolio, so the module graph stays
 * <strong>acyclic</strong> (Spring Modulith verify). The {@code internal} sub-package (the
 * aggregates, the projection and their repositories) is module-private.
 */

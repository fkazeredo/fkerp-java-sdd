 ...
 * Under ALL_SALES_FINAL the {@link ChargeKind#SUPPLIER} cost and the {@link
 * ChargeKind#CUSTOMER_REFUND} coexist — that is the merchant trap, kept visible rather than
 * collapsed into a net amount.
 *
 * @param kind the kind of obligation
 * @param amount the obligation amount (its own currency)
 * @param costBearer who bears it
 */
public record Charge(ChargeKind kind, Money amount, CostBearer costBearer) {

 ...
 * CancellationTypeCodes#ALL_SALES_FINAL} the supplier cost and the customer refund are emitted as
...
public final class CancellationCharges {
   ...
   *
   * <ul>
   *   <li><b>STANDARD/CUSTOM:</b> a single {@link ChargeKindCodes#PENALTY} from the applicable
   *       window (BR2/BR4), on {@code saleAmount}, borne by the policy's {@code costBearer}; a zero
   *       penalty produces no charge.
   *   <li><b>ALL_SALES_FINAL:</b> a {@link ChargeKindCodes#SUPPLIER} charge for the full supplier
   *       cost (irrecoverable — BR3), borne per the merchant-of-record rule (BR8); plus, when
   *       {@code refundAmount} is present, a <em>separate</em> {@link
   *       ChargeKindCodes#CUSTOMER_REFUND} charge — the two coexist (the trap, BR5).
   * </ul>
   *
  ...
  public static List<Charge> compute(
    ...
    if (CancellationTypeCodes.isAllSalesFinal(policy.type())) {
      ...
      charges.add(new Charge(ChargeKindCodes.SUPPLIER, supplierCost, bearer));
      ...
      if (refundAmount != null) {
        charges.add(new Charge(ChargeKindCodes.CUSTOMER_REFUND, refundAmount, bearer));
      }
      return charges;
    }
    ...
    if (penalty.amount().signum() > 0) {
      charges.add(new Charge(ChargeKindCodes.PENALTY, penalty, policy.costBearer()));
    }
    return charges;
  }
}

...
| 8f — Marketing (restart) | 2026-06-29 22:51 (-03:00) | — | 🟡 Em andamento. Supervisor (loop 30m) reverificou base limpa: working tree limpo, sem worktrees/branches órfãos, `develop`=`origin/develop` (`cf5eb0e`), `0.13.0`. Marcou 8f 🟡 e disparou builder em worktree isolado para `RUN-PHASE` (FASE-ALVO=8f). |
...
| **8f** | Marketing | SPEC-0019 | 🟡 In progress | Campanha/segmentação/newsletter + consentimento LGPD (cidadão de 1ª classe) + atribuição → Intelligence. Reconstrução do zero (tentativa anterior do loop 05fbf6c1 abortada por worktree dessincronizado; base limpa em `0.13.0`). Builder em worktree isolado iniciado em 2026-06-29 22:51 (-03:00). |

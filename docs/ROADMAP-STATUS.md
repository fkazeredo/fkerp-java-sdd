...
| 8f — Marketing | 2026-06-29 21:31 (-03:00) | _in progress_ | Supervisor loop (05fbf6c1): 8e ✅ (verificada, `0.13.0`, develop=origin), sem 🟡 → próxima ⬜ = **8f**; marcada 🟡 e commitada **antes** de tocar código. `RUN-PHASE` FASE-ALVO=8/sub-spec **0019** delegado a um subagente builder (worktree isolado; instruído a NÃO tocar ROADMAP-STATUS; release alvo **0.14.0**, bilíngue: pt-BR + `CHANGELOG.en-US.md`). Supervisor reverificará (`./mvnw verify`) antes de fechar. |
...
| **8f** | Marketing | SPEC-0019 | 🟡 In progress | Supervisor loop (05fbf6c1) 2026-06-29 21:31 (-03:00); `RUN-PHASE` FASE-ALVO=8, sub-spec **0019** apenas, delegado a um subagente builder. Campanha/segmentação/newsletter + consentimento LGPD (cidadão de 1ª classe) + atribuição → Intelligence. |

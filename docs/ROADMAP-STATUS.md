...
| 12 — Qualidade & E2E | 2026-06-30 17:56 (-03:00) | — | 🟡 Em andamento. Supervisor (loop 30m) reverificou base limpa: working tree limpo, sem worktrees/branches órfãos, `develop`=`origin/develop` (`2d90132`), `0.22.0`. **SPEC-0028 é nova** (builder autora primeiro): Playwright em stack isolada (`compose.e2e.yaml`, 4201, Postgres efêmero) + `@vitest/coverage-v8` + JaCoCo + sad paths + job E2E no CI; **nunca toca o banco de dev**. Tooling de qualidade, sem contrato/feature novos ⇒ **PATCH `0.22.1`** (ADR 0015). Marcou 12 🟡 e disparou builder em worktree isolado para `RUN-PHASE`. |
...
| **12** | Qualidade & E2E | SPEC-0028 (nova) | 🟡 In progress | (do fkerp-poc) Playwright em stack isolada (`compose.e2e.yaml`, 4201, Postgres efêmero) + `@vitest/coverage-v8` + JaCoCo + sad paths; job de E2E no CI; nunca toca o banco de dev. SPEC-0028 a ser autorada pelo builder. PATCH `0.22.1` (ADR 0015 — tooling, sem contrato/feature). Builder iniciado em 2026-06-30 17:56 (-03:00). |

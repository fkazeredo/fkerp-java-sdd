...
| 9 — Limpeza estrutural (internal) | 2026-06-30 06:57 (-03:00) | — | 🟡 Em andamento. Supervisor (loop 30m) reverificou base limpa: working tree limpo, sem worktrees/branches órfãos, `develop`=`origin/develop` (`f960500`), `0.20.0`. Fase **ADR+chore (sem SPEC)**: achatar `domain.<módulo>.internal.*` → `domain.<módulo>` em 20 módulos, encapsulação via `@NamedInterface`+ArchUnit (hoje 0 usos), sem mudar contrato, sem migração → **PATCH `0.20.1`** (ADR 0015). Marcou 9 🟡 e disparou builder em worktree isolado para `RUN-PHASE`. |
...
| **9** | Limpeza estrutural — remover `internal` do domain | ADR + chore | 🟡 In progress | (do fkerp-poc) Achatar `com.fksoft.domain.<módulo>.internal.*` → `…<módulo>` (**20 módulos** com `internal` hoje, main+test; herança Go). Estrutural, sem mudar contrato; encapsulação via `@NamedInterface`/ArchUnit; gates verdes. PATCH `0.20.1` (ADR 0015). Builder iniciado em 2026-06-30 06:57 (-03:00). |

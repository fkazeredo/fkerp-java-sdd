...
| 11 — Observabilidade & monitoramento | 2026-06-30 16:45 (-03:00) | — | 🟡 Em andamento. Supervisor (loop 30m) reverificou base limpa: working tree limpo, sem worktrees/branches órfãos, `develop`=`origin/develop` (`4125ed9`), `0.21.0`. **SPEC-0027 é nova** (builder autora primeiro): Micrometer + Actuator/Prometheus + logs JSON + stack Prometheus/Loki/Grafana-Alloy/Grafana via compose (`infra/`) + `GET /api/version`. Capacidade nova retrocompatível ⇒ **MINOR `0.22.0`** (ADR 0015). Marcou 11 🟡 e disparou builder em worktree isolado para `RUN-PHASE`. |
...
| **11** | Observabilidade & monitoramento | SPEC-0027 (nova) | 🟡 In progress | (do fkerp-poc) Micrometer + Actuator/Prometheus + logs JSON + Prometheus/Loki/Grafana-Alloy/Grafana via compose + `GET /api/version`. Espelhar `infra/` da POC. SPEC-0027 a ser autorada pelo builder. MINOR `0.22.0` (ADR 0015). Builder iniciado em 2026-06-30 16:45 (-03:00). |

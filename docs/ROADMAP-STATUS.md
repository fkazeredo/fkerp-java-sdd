...
| 8e — AfterSales | 2026-06-29 18:17 (-03:00) | _in progress (paused)_ | Supervisor delegou `RUN-PHASE` FASE-ALVO=8/só-0018 a um subagente; o subagente **morreu num reinício de processo** (mesmo modo de falha do 8d) deixando WIP **não commitado** e **não verificado**. O supervisor **preservou** esse WIP num checkpoint na branch `feature/8e-1-support-case-state-machine` (`d52cfa9 chore(8e): checkpoint…`). 8e segue 🟡 até o resume (retomar e terminar a partir do checkpoint, rodar `./mvnw verify`, então fechar). |

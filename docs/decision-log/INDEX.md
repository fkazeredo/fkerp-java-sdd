...
| [DL-0017](DL-0017-inbound-account-not-found-rejects.md) | Inbound: Account inexistente **rejeita** (422) — **REVISADA na Fase 19b (ver DL-0120): o 422 permanece, mas o payload agora é preservado em quarentena com replay** | **Baixa** | Moderada | Decisão de negócio em aberto na SPEC-0009; só o dono fecha (criar conta provisória segue vetado) |
...
| [DL-0044](DL-0044-billing-tax-regime-simples-and-swappable-strategy.md) | Billing: regime **Simples Nacional** (default) + estratégia trocável de ISS/retenções (Q7) — **REFINADA na Fase 19b (ver DL-0121): nuance Anexo III×V/Fator R registrada + flag `regime-confirmed` como gate de emissão real** | **Baixa** | **Cara** | **Q7 — regime tributário/quem emite é incógnita de negócio (só o contador fecha)**; move a tese tributária e o impacto fiscal de notas já emitidas é externo e caro |
...
| [DL-0116](DL-0116-enum-to-cadastro-marketing-intelligence-portfolio.md) | 18b | enum→cadastro **Marketing/Intelligence/Portfolio** (V34): `ConsentPurpose`/`SubjectType`, `SubjectKind`/`InsightType`/`Verdict`, `GoalMetric` viram codes; `INSIGHT_*` produzidos pelo sistema (sem validação de escrita); rótulo nas telas via `CadastroLabelPipe` | Alta | Moderada |
| [DL-0117](DL-0117-enum-to-cadastro-sourcing-exchange-booking-compliance.md) | 18c | enum→cadastro **Sourcing/Exchange/Booking/Compliance** (V35): `OfferOrigin`/`IntegrationLevel`, `MarketRateSource`, `ChargeKind`/`CancellationType`, `DocumentType`/`SignedFormat`/`RequirementPhase` viram codes; ramificação preservada (INTEGRATED/armadilha do lojista/retenção); `MarketRateSource`/`SignedFormat` produzidos (sem validação de escrita); rótulo nas 4 telas | Alta | Moderada |
| [DL-0118](DL-0118-enum-to-cadastro-finance-payout-people-commercialpolicy-aftersales.md) | 18d | enum→cadastro **Finance/Payout/People/CommercialPolicy/AfterSales** (V36) — **FECHA a Fase 18**: `EntryType`/`PartyType`, `PayeeType`/`PayoutKind`, `DiscrepancyKind`, `ParameterValueType`, `SupportCaseType`/`CaseResolution` viram codes; ramificação preservada (postagem AP/AR, fato do payout/armadilha do lojista, orquestração do CaseResolution, parse do value-type); `DiscrepancyKind` produzido (sem validação de escrita); `LedgerDirection`/`ParameterLayer` **mantidos enum** (keep documentado); rótulo nas 5 telas | Alta | Moderada |
| [DL-0119](DL-0119-authorization-matrix-default-deny.md) | 19a | Autorização: **matriz única rota×papel** (`ApiAuthorizationMatrix`) com **default-deny** para escritas não mapeadas + completude como portão de build; `permitAll` de `/api/integration/**` estreitado aos 2 M2M com HMAC (AFD/crawl exigem `ROLE_IT`); leituras de dados pessoais (People/Ponto) → IT; VIEWER não escreve | Alta | Barata |
| [DL-0120](DL-0120-inbound-quarantine-replay.md) | 19b | Inbound rejeitado vai para **quarentena** com replay/descarte operacional (exception queue; V37; transação própria; idempotente por id externo) — **revisa a DL-0017** mantendo o contrato externo (422, nada no núcleo, sem conta provisória) | Alta | Barata |
| [DL-0121](DL-0121-billing-anexo-fator-r-and-emission-confirmation-flag.md) | 19b | Billing: nuance do **enquadramento** (agência CNAE 7911 → Anexo III sem Fator R; **representação comercial/GSA → Anexo V com Fator R**) registrada para o contador + flag **`billing.tax.regime-confirmed`** (default false) como gate de emissão real em produção (enforcement na 19c) — **refina a DL-0044** | Alta | Barata |
| [DL-0122](DL-0122-webhook-anti-replay-timestamp.md) | 19c | Webhooks: assinatura passa a cobrir **`timestamp + "." + body`** com **janela anti-replay** (default 300s) num **helper único** (`WebhookSignatures`); header de timestamp exigido nos 2 webhooks (breaking M2M) | Alta | Moderada |
| [DL-0123](DL-0123-prod-readiness-fail-fast.md) | 19c | **Fail-fast de produção** (`ProdReadinessValidator`, perfil `prod`): recusa subir com segredo default/vazio (webhooks, chave do cofre, senha de DB), issuer `http://` ou regime tributário não confirmado | Alta | Barata |
| [DL-0124](DL-0124-vault-upload-magic-bytes.md) | 19c | Cofre: upload valida **magic bytes** dos binários (não confia na extensão — fecha gap doc×código) + `fileRef` validado como UUID em read/delete | Alta | Barata |
| [DL-0125](DL-0125-login-lockout-and-password-policy.md) | 19c | Login: **lockout de força-bruta** DB-backed (5 falhas → 15 min, V38) via `accountLocked` no `UserDetailsService` + **política mínima de senha** (≥8, não-repetido); erro genérico (BR4) | Alta | Moderada |
| [DL-0126](DL-0126-real-api-docs-tag-global-errors-pkce-snapshot.md) | 19d | **Doc de API real**: `@Tag` nos 37 controllers (+ fitness ArchUnit), contrato de erro documentado globalmente (`OpenApiCustomizer`), botão **Authorize** OAuth2 Code+PKCE ao AS self-hosted, `Info` enxuta, **snapshot `docs/api/openapi.json` + teste de drift** | Alta | Barata |
| [DL-0127](DL-0127-integration-emulators-http-adapter.md) | 19e | **Emulação de integração**: outbound NFS-e graduado para **adaptador HTTP real** (`HttpMunicipalNfseService`, timeout/retry/`OutboundCircuitBreaker`) selecionável por `billing.nfse.adapter` (default `simulated`); **emulador em dev** (WireMock no compose, profile `emulators`) **e em testes** (JDK `HttpServer`); seam p/ os demais mocks | Alta | Barata |
| [DL-0128](DL-0128-spring-boot-4.1-jackson-3-remove-bridge.md) | 19f | **Spring Boot 4.0.7→4.1.0 + Modulith 2.1.0 + Jackson 2→3** (`tools.jackson`), **remove a ponte `starter-classic`** (quita o débito da DL-0108); Flyway passa ao **starter modular** (`spring-boot-starter-flyway`); contrato byte-idêntico (snapshot OpenAPI). Pedido do dono | Alta | Moderada |
| [DL-0129](DL-0129-multi-instance-jwk-jdbc-session.md) | 19g | **Multi-instância/HA (ADR-0020, revisa o ADR-0002):** chave de assinatura **persistida** (`OIDC_JWK_PRIVATE_KEY` — restart não desloga; exigida em prod), **estado do AS em JDBC** (V39, client SPA bootstrap idempotente) e **sessão do form-login via Spring Session JDBC**; breakers seguem por instância (decisão) | Alta | Moderada |
| [DL-0130](DL-0130-fx-forwards-unhedged-drift-alert.md) | 19h | **Hedge cambial**: `ForwardContract` manual (V40, `OPEN → SETTLED/CANCELLED`), cobertura por moeda no `LiveExposure` (`openForwards`/`unhedgedExposureBase`) e **alerta de drift sobre o descoberto** (revisa a DL-0027 — livro 100% coberto não alerta); escritas DIRECTOR/FINANCE; `HedgeAdvisor` adiado à Fase 20c | Alta | Barata |
| [DL-0131](DL-0131-concurrency-period-lock-optimistic-409.md) | 19i | **Concorrência**: `register`/`postFromCharge` tomam o **lock de linha do período** do `closePeriod` (furo real: entrada escorregava para o período recém-selado — regressão vermelho→verde) + **conflito otimista → 409 `error.conflict`** (antes 500); corridas de payout/close provadas com testes determinísticos | Alta | Barata |
| [DL-0132](DL-0132-mutation-testing-jqwik-branch-floors.md) | 19i | **Portões elevados**: **PIT** em profile `mutation` sobre a matemática de dinheiro (185 mutantes, 68% mortos, threshold 60, job CI próprio) + **jqwik** (propriedades de `Money`/`InstallmentPlan`, 1000 casos cada) + **pisos** JaCoCo BRANCH ≥ 0,65 e Vitest 70/75/49 (branches mantém 55) | Alta | Barata |
